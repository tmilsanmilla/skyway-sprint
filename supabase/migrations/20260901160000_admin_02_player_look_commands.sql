-- Admin 02 Player Look and Commands
--
...
-- Prefer immutable user IDs for admin checks. The email fallback is retained
-- only for a legacy admin row that has not yet been linked to auth.users.
  ...
  select auth.uid() is not null and exists (
    ...
    where (admin.user_id is not null and admin.user_id = auth.uid())
       or (
         ...
         and admin.email = lower(coalesce(auth.jwt()->>'email', ''))
       )
  );
$$;
...
as $$
  select auth.uid() is not null and exists (
    ...
    where admin.role = 'main'
      and (
        (admin.user_id is not null and admin.user_id = auth.uid())
        or (
          admin.user_id is null
          and admin.email = lower(coalesce(auth.jwt()->>'email', ''))
        )
...
-- `20260901155000_shop_refund_and_ledger.sql` creates the append-only future
-- extraction ledger and instruments extract_items before this migration runs.
-- Admin 02 exposes those rows only through its role-checked player detail RPC.
...
-- Recheck administrator roles after the ban tables exist. This ensures an
-- account ban also removes every admin capability until the ban expires or is
-- revoked, including capabilities exposed by older admin RPCs that delegate to
...
as $$
  select admin.role
  from public.admin_users admin
  where not app_private.has_active_ban(auth.uid(), 'account', null)
    and (
      (admin.user_id is not null and admin.user_id = auth.uid())
      or (
        admin.user_id is null
...
begin
  ...
  select jsonb_build_object(
    'account', (
      ...
      from auth.users users
      left join public.admin_users admin
        on admin.user_id = users.id
        or (admin.user_id is null and admin.email = lower(users.email))
    ...
    'unlocks', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'display_name', coalesce(catalog.display_name, initcap(replace(unlock.item_key, '_', ' '))),
    ...
    'devices', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'device_banned', app_private.has_active_ban(p_user_id, 'device', device.id)
      ) order by link.last_seen_at desc)
    ...
    'multiplayer', (
      select jsonb_build_object(
        'matches_played', count(*)::bigint,
        'wins', count(*) filter (
          where match.winner_user_id = p_user_id
        )::bigint,
        'best_wave', coalesce(max(player.wave), 0),
        'best_match_score', coalesce(max(player.score), 0),
        'coins_collected', coalesce(sum(player.melons_collected), 0),
        'active_match', (
          select jsonb_build_object(
            'match_id', active_player.match_id,
            'status', active_match.status,
            'wave', active_player.wave,
            'score', active_player.score,
            'coins', active_player.obstacle_points,
            'hearts', active_player.hearts
          )
          from public.multiplayer_players active_player
          join public.multiplayer_matches active_match
            on active_match.id = active_player.match_id
          where active_player.user_id = p_user_id
            and active_match.status in ('countdown', 'playing', 'intermission')
          order by active_match.created_at desc
          limit 1
        )
      )
